...
name <- "UKHSA_v3_20220301_AZPD2=FALSE_SB=FALSE_NewDecay=TRUE_AddBst=FALSE"

###################################
### parameters from MCMC chain  
###################################

load(paste0("0_antibody_model_fitting/Model Fits/",name,"_mcmc_chain.Rdata"))
...
# changed back to using hospitalisations since we might want to focus on those given that our vaccines
# are not capturing additional protection against death? can decide later
         # ab50_s = ab50_d,
         om_red = 10^(om_red),
  ...
  select(-ni50, -ns50, -nd50, -d2_AZ_log10, -d2_PF_log10, -d2_MD_log10, -bst_AZ, -bst_PF, -bst_MD)
...
mu_ab_d1 <- c(pm$d1_AZ, pm$d1_PF, pm$d1_MD)
mu_ab_d2 <- c(pm$d2_AZ, pm$d2_PF, pm$d2_MD)
mu_ab_d3 <- c(pm$d3_AZ, pm$d3_PF, pm$d3_MD)
k <- pm$k
hl_s <- pm$hl_s
hl_l <- pm$hl_l
