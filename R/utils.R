...
get_vaccine_pars <- function(
  ...
  if (omicron_vaccine==1) {
    dose_5_fold_increase <- vfr 
  } else {
    dose_5_fold_increase <- 1
  }
  ...
  mu_ab_list <- data.frame(name = vaccine,
    ...
    mutate(mu_ab_d5 = mu_ab_d4 * dose_5_fold_increase) %>%
    mutate(mu_ab_d6 = mu_ab_d4 * dose_5_fold_increase) %>%
    mutate(mu_ab_d7 = mu_ab_d4 * dose_5_fold_increase) %>%
    mutate(mu_ab_d8 = mu_ab_d4 * dose_5_fold_increase) %>%
    mutate(mu_ab_d9 = mu_ab_d4 * dose_5_fold_increase)
