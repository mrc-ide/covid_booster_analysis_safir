...
rt_out_filename <- "rt_out_hic_zerocovid_newvariant"
lambda_external_filename <- "lambda_external_hic_zerocovid"
...
scenarios <- expand_grid(fit = fit,
  ...
  filter((vaccine_doses==3 & age_groups_covered_d4 == 5)| (vaccine_doses == 6) | (vaccine_doses ==9 & age_groups_covered_d4 <= 5)) %>%
  ...
  mutate(rt_out_filename = if_else((vfr2 == vfr & hosp_scal_vfr2 == hosp_scal_vfr), "rt_out_hic_zerocovid", rt_out_filename)) %>%
  unique()
