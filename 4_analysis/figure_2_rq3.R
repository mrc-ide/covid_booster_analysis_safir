...
rq3_hosp <- ggplot(data = df1, aes(x = as.Date(date), y = hosp_t/target_pop * 1e6, col = strategy_name)) +
  ...
  labs(x = "Time", y = "Daily hospital admissions per million", col = "Dose strategy", fill = "Dose strategy")
rq3_hosp
#ggsave("plots/rq3_hosp.png", rq3_hosp, height = 4, width = 10)
...
rq3_inc
#ggsave("plots/rq3_incidence.png", rq3_inc, height = 4, width = 10)

rq3_sp <- ggplot(data = df1, aes(x = as.Date(date), y = sp_med, col = strategy_name)) +
  geom_line() +
  theme_bw() +
  theme(strip.background = element_rect(fill = NA),
        panel.border = element_blank(),
...
rq3_sp
#ggsave("plots/rq3_sp.png", rq3_sp, height = 4, width = 10)
...
combined
ggsave("plots/fig2_rq3.png", combined, height = 13, width = 13)
