...
repetition <- 1:20
vacc_start <- "4/1/2021"
vaccine_doses <- c(2,3,4)
max_coverage <- 0.8
age_groups_covered <- 9
seeding_cases <- 10
vacc_per_week <- 0.02
t_d3 <- 365 
strategy <- c("same_doses", "two_doses_only", "annual_boost")
...
mu_ab_infection <- 1.2
max_ab <- 5
rt_out_filename <- "rt_out_lmic_newvar"
lambda_external_filename <- "lambda_external_lmic"
...
scenarios <- expand_grid(fit = fit,
  ...
  filter((strategy == "same_doses" & vaccine_doses %in% c(2,3)) | (strategy == "two_doses_only" & vaccine_doses == 2) | (strategy == "annual_boost" & vaccine_doses == 4)) %>%
  ...
  mutate(rt_out_filename = if_else((vfr2 == vfr & hosp_scal_vfr2 == hosp_scal_vfr), "rt_out_lmic", rt_out_filename)) %>%
  unique()
